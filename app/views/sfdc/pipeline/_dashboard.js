/**
 * Pipeline Dashboard Initialization Script
 *
 * Fetches dashboard data from /api/sfdc_pipeline.php?action=get_dashboard_data
 * Renders 6 Chart.js stacked bar charts (All/Fixed/ICT x Amount/Expected_Revenue)
...
(function (window, document) {
    ...
    const DashboardConfig = {
        endpoint: '../api/sfdc_pipeline.php?action=get_dashboard_data',
        charts: {},
        currentData: null,
        initialized: false,
        loaded: false,
    ...
    function populateFiscalYears(currentYear) {
        const dropdown = document.getElementById('dashboardFiscalYearPipeline');
        if (!dropdown) return;
    ...
    function updateKpiDisplays(data) {
        ...
        // All metrics
        setText('kpiAllAmountTotal', formatCurrency(allKpi.total_amount || 0));
        setText('kpiAllAmountAvg', formatCurrency(allKpi.avg_amount || 0));
        setText('kpiAllAmountDeals', allKpi.deal_count || 0);

        setText('kpiAllExpRevTotal', formatCurrency(allKpi.total_expected_revenue || 0));
        setText('kpiAllExpRevAvg', formatCurrency(allKpi.avg_expected_revenue || 0));
        setText('kpiAllExpRevDeals', allKpi.deal_count || 0);

        // Fixed metrics
        setText('kpiFixedAmountTotal', formatCurrency(fixedKpi.total_amount || 0));
        setText('kpiFixedAmountAvg', formatCurrency(fixedKpi.avg_amount || 0));
        setText('kpiFixedAmountDeals', fixedKpi.deal_count || 0);

        setText('kpiFixedExpRevTotal', formatCurrency(fixedKpi.total_expected_revenue || 0));
        setText('kpiFixedExpRevAvg', formatCurrency(fixedKpi.avg_expected_revenue || 0));
        setText('kpiFixedExpRevDeals', fixedKpi.deal_count || 0);

        // ICT metrics
        setText('kpiIctAmountTotal', formatCurrency(ictKpi.total_amount || 0));
        setText('kpiIctAmountAvg', formatCurrency(ictKpi.avg_amount || 0));
        setText('kpiIctAmountDeals', ictKpi.deal_count || 0);

        setText('kpiIctExpRevTotal', formatCurrency(ictKpi.total_expected_revenue || 0));
        setText('kpiIctExpRevAvg', formatCurrency(ictKpi.avg_expected_revenue || 0));
        setText('kpiIctExpRevDeals', ictKpi.deal_count || 0);
    }

    function buildChartDatasets(data, type, metric) {
        const metricKey = metric === 'amount' ? 'amount' : 'expected_revenue';
        const datasets = [];
    ...
    function showState(state, errorMessage) {
        const loadingEl = document.getElementById('dashboardLoadingPipeline');
        const contentEl = document.getElementById('dashboardContentPipeline');
        const errorEl = document.getElementById('dashboardErrorPipeline');
        const errorMsgEl = document.getElementById('dashboardErrorMessagePipeline');
    ...
    function renderAllCharts(data) {
        destroyAllCharts();
        renderChart('chartAllAmount', data, 'All', 'amount');
        renderChart('chartAllExpRev', data, 'All', 'expected_revenue');
        renderChart('chartFixedAmount', data, 'Fixed', 'amount');
        renderChart('chartFixedExpRev', data, 'Fixed', 'expected_revenue');
        renderChart('chartIctAmount', data, 'ICT', 'amount');
        renderChart('chartIctExpRev', data, 'ICT', 'expected_revenue');
    }
    ...
    function isDashboardTabVisible() {
        const tab = document.getElementById('pipeline-dashboard-tab');
    ...
    function waitForVisibleContent(callback, attempts) {
        ...
        const content = document.getElementById('dashboardContentPipeline');
    ...
    function onDashboardShown() {
        if (!DashboardConfig.loaded) {
            const fyDropdown = document.getElementById('dashboardFiscalYearPipeline');
    ...
    function bindEvents() {
        const yearDropdown = document.getElementById('dashboardFiscalYearPipeline');
        if (yearDropdown) {
        ...
        const refreshBtn = document.getElementById('dashboardRefreshPipeline');
        if (refreshBtn) {
    ...
    function initDashboard() {
        ...
        const dashboardTab = document.getElementById('pipeline-dashboard-tab');
